...
class ChangeIterator:
    ...
    def _get_changesets(self, events):
        ...
        for model in self.models:
            ...
            if model_obj and model_obj._event in events:
                ...
                # build changeset
                changeset = view_obj.data_changeset(
                    self.cr,
                    self.uid,
                    model_obj._name,
                    schema=model_schema,
    ...
    def next(self):
        ...
        for model in self.models:
            model_obj = self.pool.get(model)
            if model_obj and (
                not model_obj._chgnotify_enabled or self.state == STATE_PREPARED
            ):
...
class DataSet(http.Controller):
    ...
    def changes(self, models=None, model=None, params=None, timeout=30, receiver=None):
        ...
        if models:
            chg_gen = ChangeIterator(
                pool, request.uid, models, params, timeout, receiver
            )
