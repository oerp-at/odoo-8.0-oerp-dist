...
class invoice_merge_wizard(osv.osv_memory):
    def default_get(self, cr, uid, fields_list, context=None):
        res = super(invoice_merge_wizard, self).default_get(
        ...
        for invoice in invoice_obj.browse(cr, uid, invoice_ids, context):
            ...
            if not data:
                data = {
                    ...
                    and invoice.payment_term.id
                    or None,
                    "fiscal_position": invoice.fiscal_position
                    and invoice.fiscal_position.id
                    or None,
            ...
            dt_invoice = (
                invoice.date_invoice and util.strToDate(invoice.date_invoice) or None
            )
            ...
            if invoice.state == "draft":
                ...
                if dt_invoice:
                    dt_from_date = (
                        dt_from_date and min(dt_from_date, dt_invoice)
                    ) or dt_invoice
                    dt_to_date = (
                        dt_to_date and max(dt_to_date, dt_invoice)
                    ) or dt_invoice

                data_num += 1
            else:
                data_state = addInfo(
                    1, _("%s is no draft Invoice, Ignored!") % (invoice.name,)
                )
        ...
        for data in invoice_merge.values():
            mergeLines("info")
            mergeLines("comment")
            str_range = helper.getMonthYearRange(
                cr, uid, toStrDate("from_date"), toStrDate("to_date"), context
            )
            if str_range:
    ...
    def _prepare_invoice(self, cr, uid, merge, context=None):
        doc = {
            ...
            "fiscal_position": merge.fiscal_position
            and merge.fiscal_position.id
            or None,
    ...
    def do_merge(self, cr, uid, ids, context=None):
        ...
        has_analytic_invoice = (
            account_analytic_line_obj
            and account_analytic_line_obj.fields_get(cr, uid, allfields=["invoice_id"])
            and True
        ) or False
        ...
        for wizard in self.browse(cr, uid, ids, context):
            for merge in wizard.merge_ids:
                ...
                for invoice in merge.invoice_ids:
                    ...
                    for line in invoice.invoice_line:
                        ...
                        if invoice.origin and (
                            not origin or invoice.origin not in origin
                        ):
                            origin = (
                                origin
                                and ":".join((invoice.origin, origin))
                                or invoice.origin
                                or None
                            )
                        ...
                        invoice_line_obj.write(
                            ...
                            uid,
                            line.id,
                            {"origin": origin, "invoice_id": invoice_id},
                            context=context,
                        )

                # correct work flows
                cr.execute(
                    "SELECT id FROM wkf_instance WHERE res_type='account.invoice' AND res_id=%s",
                    (invoice_id,),
                )
                ...
                # update work items
                if work_item_ids:
                    wkf_workitem_obj.write(
                        cr, uid, work_item_ids, {"subflow_id": new_wkf_inst_id}
                    )
                ...
                if invoice_ids:
                    ...
                    if purchase_obj:
                        ...
                        purchase_obj.write(
                            cr, uid, depIds, {"invoice_ids": [(4, invoice_id)]}, context
                        )
                    ...
                    if has_analytic_invoice:
                        ...
                        account_analytic_line_obj.write(
                            cr, uid, depIds, {"invoice_id": invoice_id}, context
                        )

                    invoice_obj._replace_invoice_ids_with_id(
                        cr, uid, invoice_ids, invoice_id, context
                    )
