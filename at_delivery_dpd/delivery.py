...
class delivery_carrier(osv.Model):
    ...
    _dpd_errors = [
        ...
        (
            "ER13",
            "Kundennummer und/oder Bezugsperson fehlt (Pflichtig für PaketShop Zustellung)",
        ),
        ...
        (
            "PR02-hv",
            "Falscher oder fehlender Wert für Zusatzleistung Höherversicherung",
        ),
        (
            "PR02-asg",
            "Falscher oder fehlender Wert für Zusatzleistung Abstellgenehmigung",
        ),
        (
            "PR02-id",
            "Falscher oder fehlender Wert für Zusatzleistung Identitätsprüfung",
        ),
        (
            "PR02-abt",
            "Falscher oder fehlender Wert für Zusatzleistung Abteilungsbelieferung",
        ),
        ...
        (
            "PR03-nachnahme",
            "Falscher oder fehlender Wert für Zusatzleistung DPD Nachnahme",
        ),
        (
            "PR03-nnbar",
            "Falscher oder fehlender Wert für Zusatzleistung primetime Nachnahme",
        ),
        ...
        (
            "FR04",
            "Übergebene Paketscheinnummer für Paketscheinstorno ist nicht korrekt oder nicht vorhanden.",
        ),
    ...
    def _dpd_client_get(self, context=None):
        if self._dpd_client is None:
            self._dpd_client = Client(
                "http://web.paketomat.at/webservice/service-1.0.2.php?wsdl"
            )
            ...
            for packageNo in range(0, package_count):
                ...
                if weight:
                    ...
                    uom_id = uom_obj.search_id(
                        ...
                        uid,
                        [
                            ("category_id", "=", picking.weight_uom_id.category_id.id),
                            "|",
                            ("name", "=", "g"),
                            ("code", "=", "g"),
                        ],
                    ...
                    parts["gewicht"] = str(
                        int(
                            uom_obj._compute_qty(
                                cr, uid, picking.weight_uom_id.id, weight, uom.id
                            )
                        )
                    )
                ...
                if label_url:
                    ...
                    try:
                        # add page
                        label_pdf.addPage(
                            PdfFileReader(StringIO(label_file.read())).getPage(0)
                        )
                    finally:
                        label_file.close()
    ...
    def _dpd_cancel(self, cr, uid, picking, context=None):
        ...
        try:
            if picking.carrier_tracking_ref:
                for tracking_ref in picking.carrier_tracking_ref.split(", "):
                    ...
                    if msgSoapOut.storno != "1" and msgSoapOut.err_code != "FR06":
                        raise Warning(
                            _("Unable to cancel %s. Call DPD for manual cancel!")
                            % tracking_ref
                        )
            ...
            picking_obj.write(
                ...
                uid,
                picking.id,
                {"carrier_status": None, "carrier_error": None},
                context=context,
            )
    ...
    _columns = {
        ...
        "dpd_test_profile_id": fields.many2one(
            "delivery.carrier.dpd", "DPD Test Profile"
        ),
        "dpd_type": fields.selection(
            [
                ("DPD", "DPD"),
                ("PT", "Prime Time"),
                ("B2C", "B2C"),
