...
class commission_line(osv.osv):
    ...
    def _update_bonus(self, cr, uid, salesman_ids, period_ids, context=None):
        ...
        cr.execute(
            "SELECT s.id team_id, cl.period_id, cl.partner_id, SUM(cl.price_sub) "
    ...
    @api.cr_uid_context
    def _validate_sale_commission(
        self, cr, uid, values, obj=None, company=None, context=None
    ):
        return values

    @api.cr_uid_context
    def _get_sale_commission(
        self,
        cr,
        uid,
        name,
        ...
        if commission_custom is None:
            ...
            if rule:
                ...
                item_id = pricelist_obj.price_rule_get(
                    cr,
                    uid,
                    [pricelist.id],
                    product.id,
                ...
                if item_id:
                    prule = pricelist_item_obj.read(
                        cr,
                        uid,
                        item_id,
                        ["commission_active", "commission"],
                        context=context,
                    )
        ...
        if percent:
            ...
            entry = self._validate_sale_commission(
                cr, uid, entry, obj=obj, company=company, context=context
            )
            if entry:
                res.append(entry)
    ...
    _columns = {
        ...
        "sales_bonus_line_id": fields.many2one(
            "commission_sale.bonus_line", "Sales Bonus", ondelete="restrict"
        ),
    }
