...
class sale_order_edit_wizard_line(models.TransientModel):
    ...
    route_id = fields.Many2one(
        "stock.location.route", "Route", domain=[("sale_selectable", "=", True)]
    )
            ...
            if self.env.user.has_group("base.group_sale_manager"):
                ...
                for line in modify_wizard.line_ids:
                    if modify_wizard.modify:
                        ...
                        # get line values
                        line_values = sale_line.product_id_change_with_wh_price(
                        ...
                        if (
                            new_route.id != cur_route.id
                            and sale_line.need_procurement()
                        ):
                            ...
                            procurement_vals = order._prepare_order_line_procurement(
                                order, sale_line, group_id=order.procurement_group_id.id
                            )
                ...
                if modify_wizard.modify:
                    ...
                    modify_wizard.order_id.partner_shipping_id = (
