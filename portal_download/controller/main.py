...
class portal_download_ctrl(http.Controller):
    @http.route(
        ["/portal_download/start/<int:download_id>"],
        type="http",
        auth="public",
        methods=["GET"],
    )
    def download_start(self, download_id=None, **kwargs):
        ...
        if download_key:
            ...
            if perm_ids:
                ...
                uid = user_obj.search_id(
                    cr, SUPERUSER_ID, [("partner_id", "=", perm.partner_id.id)]
                )
                if not uid:
        ...
        att_ids = att_obj.search(
            cr,
            uid,
            [("res_model", "=", "portal.download"), ("res_id", "=", download.id)],
            limit=1,
        )
        if not att_ids:
            return request.not_found()
        ...
        att_mimetype = (
            mimetypes.guess_type(att.datas_fname or "")[0] or "application/octet-stream"
        )
        ...
        if perm_ids:
            for perm_id in perm_ids:
                download_count = perm_obj.read(
                    cr, SUPERUSER_ID, perm_id, ["download_count"], context=context
                )["download_count"]
                perm_obj.write(
                    cr,
                    SUPERUSER_ID,
        ...
        return request.make_response(
            file_out,
            [("Content-Type", att_mimetype), ("Content-Disposition", att_disp)],
        )
