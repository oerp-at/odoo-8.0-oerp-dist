...
class sale_discount_wizard(models.TransientModel):
    ...
    amount = fields.Float(
        "Amount", digits_compute=dp.get_precision("Account"), required=True
    )

    total = fields.Float(
        "Total", digits_compute=dp.get_precision("Account"), default=_default_total
    )
    ...
    order_id = fields.Many2one(
        ...
        "Order",
        required=True,
        default=lambda self: util.active_id(self._context, "sale.order"),
    )
        ...
        for wizard in self:
            if wizard.type == "reset":
                ...
                if not total or not apply_to_lines:
                    raise Warning(
                        _("Unable to apply overall discount for amount %s and lines %s")
                        % (total, len(apply_to_lines))
                    )

                if wizard.type == "total":
                    discount = 100.0 - (
                        100.0 / total * (wizard.amount - discount_price)
                    )
                elif wizard.type == "discount":
                    discount = 100.0 - (
                        100.0 / total * (total - wizard.amount - discount_price)
                    )
                else:
