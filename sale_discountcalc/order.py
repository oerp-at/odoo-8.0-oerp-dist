...
class sale_order_line(osv.Model):
    ...
    _re_discount_price = re.compile(
        "^[" + re.escape("=-*") + "]\\s*" + NUMBER_PATTERN + "\\s*$"
    )
    ...
    def _discount_calc_get(self, cr, uid, ids, field_name, arg, context=None):
        ...
        f_price = format.LangFormat(
            cr, uid, context=context, obj=self, f="discount_price"
        )
    ...
    def _discount_calc_set(
        self, cr, uid, id, field_name, field_value, arg, context=None
    ):
        discount_price = 0.0
        discount = 0.0
        ...
        if field_value:
            ...
            if not discount_price is None:
                values = self.read(
                    cr, uid, id, ["price_unit", "product_uom_qty"], context=context
                )
                ...
                if total:
                    if field_value[0] == "*":
                        ...
                        discount = 100.0 - (
                            (100.0 / (qty * price_unit)) * (qty * discount_unit_price)
                        )
                    else:
                        discount = 100.0 - (
                            (100.0 / (qty * price_unit)) * discount_price
                        )
            else:
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if (
            not "discount_calc" in vals
            and "price_unit" in vals
            or "product_uom_qty" in vals
        ):
            for stored_vals in self.read(
                cr, uid, ids, ["discount_calc"], context=context
            ):
                discount_calc = stored_vals.get("discount_calc")
                if discount_calc:
                    self._discount_calc_set(
                        cr,
                        uid,
                        stored_vals["id"],
                        "discount_calc",
    ...
    _columns = {
        "discount_unit": fields.boolean(
            "Unit Discount", readonly=True, states={"draft": [("readonly", False)]}
        ),
        "discount_action": fields.boolean(
            "Action", states={"draft": [("readonly", False)]}
        ),
        ...
        "discount_calc": fields.function(
            _discount_calc_get,
            fnct_inv=_discount_calc_set,
            type="char",
            string="Discount",
            readonly=True,
            states={"draft": [("readonly", False)]},
