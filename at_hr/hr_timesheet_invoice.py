...
class account_analytic_line(osv.Model):
    def _prepare_cost_invoice(
        self, cr, uid, partner, company_id, currency_id, analytic_lines, context=None
    ):
        res = super(account_analytic_line, self)._prepare_cost_invoice(
        ...
        cr.execute(
            "SELECT MIN(line.date), MAX(line.date) "
            "FROM account_analytic_line as line "
            "WHERE account_id = %s "
            "AND id IN %s AND to_invoice IS NOT NULL",
            (account.id, tuple(line_ids)),
        )
        ...
        for date_from, date_to in cr.fetchall():
            if date_from and date_to:
                invoice_name = "%s %s - %s" % (
                    account.name or "",
                    f.formatLang(date_from, date=True),
                    f.formatLang(date_to, date=True),
                )

        if not invoice_name:
            invoice_name = "%s %s" % (
                account.name or "",
                f.formatLang(util.currentDate(), date=True),
            )
    ...
    def _prepare_cost_invoice_line(
        ...
        if product_id:
            ...
            if factor.customer_name:
                ...
                general_account = (
                    product.property_account_income
