...
class Parser(extreport.basic_parser):
    ...
    def _invoice_lines(self, invoice):
        ...
        ids = invoice_line_obj.search(
            self.cr, self.uid, [("invoice_id", "=", invoice.id)]
        )
        for i in range(0, len(ids)):
            lines = invoice_line_obj.browse(
                self.cr, self.uid, ids[i], self.localcontext
            )
            invoice_lines.append(lines)

        line_format = invoice_line_obj._line_format(
