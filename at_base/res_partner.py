...
class res_partner(osv.osv):
    def _check_for_ref(self, cr, uid, vals, context=None):
        if not vals.get("ref", False) and (
            vals.get("customer", False) or vals.get("supplier", False)
        ):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if not has_name and ("surname" in vals or "firstname" in vals):
            ...
            for partner_vals in self.read(
                cr, uid, ids, ["name", "is_company", "surname", "firstname"]
            ):
                if not vals.get("is_company", partner_vals.get("is_company")):
                    ...
                    super(res_partner, self).write(
                        cr, uid, [partner_vals["id"]], new_vals, context=context
                    )
                else:
                    # write default
                    super(res_partner, self).write(
                        cr, uid, [partner_vals["id"]], vals, context=context
                    )
    ...
    def _build_address_text(
        ...
        if not without_name:
            if address_type == "mail":
                ...
                _lines.append(
                    title
                    and "\n".join((title.name, _name_address.name))
                    or _name_address.name
                )
                # build co
    ...
    def _get_mail_address(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for partner in self.browse(cr, uid, ids, context):
            res[partner.id] = "\n".join(
                self._build_address_text(
                    cr, uid, partner, address_type="mail", context=context
                )
            )
        return res
    ...
    def _get_display_address(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for partner in self.browse(cr, uid, ids, context):
            res[partner.id] = "\n".join(
                self._build_address_text(cr, uid, partner, context=context)
            )
        return res
    ...
    def _get_login_id(self, cr, uid, ids, field_name, arg, context=None):
        res = dict.fromkeys(ids)
        cr.execute(
            "SELECT p.id, u.id FROM res_users u "
    ...
    def name_search(
        ...
        if not res:
            ids = self.search(
                cr,
                uid,
                ["|", ("ref", operator, name), ("category_id.name", operator, name)],
                limit=limit,
                context=context,
            )
            if ids:
    ...
    _columns = {
        ...
        "mail_salutation": fields.function(
            _get_mail_salutation, type="text", string="Mail Salutation"
        ),
        "mail_address": fields.function(
            _get_mail_address, type="text", string="Mail Address"
        ),
        "display_address": fields.function(
            _get_display_address, type="text", string="Address"
        ),
        "contact_info": fields.function(
            _get_contact_info, type="text", string="Contact Info"
        ),
        "co_salutation": fields.function(
            _get_co_salutation, type="text", string="C/O Salutation"
        ),
        "phone_n": fields.function(
            _get_phone, type="char", store=True, string="Phone normalized"
        ),
        "mobile_n": fields.function(
            _get_mobile, type="char", store=True, string="Mobile normalized"
        ),
        "fax_n": fields.function(
            _get_fax, type="char", store=True, string="Fax normalized"
        ),
        "birthday": fields.date("Birthday"),
        "firstname": fields.char("First Name"),
        "surname": fields.char("Last Name"),
        "login_id": fields.function(
            _get_login_id,
            string="Login",
            type="many2one",
            obj="res.users",
