...
class res_mapping(osv.Model):
            ...
            if not res_model:
                ...
                if not uuid_id:
                    raise osv.except_osv(
                        _("Error"), _("No model found for uuid %s" % uuid)
                    )
    ...
    def get_mapping(self, cr, uid, res_uuid, res_model=None, name=None):
        ...
        if not res_model:
            uuid_id = self.search_id(
                cr, uid, [("name", "=", name), ("uuid", "=", res_uuid)]
            )
        else:
            uuid_id = self.search_id(
                cr,
        ...
        if not uuid_id:
            return {"res_id": None, "res_model": res_model}

        values = self.read(
            cr, uid, uuid_id, ["res_id", "res_model", "locked", "active"]
        )
        res_id = self.pool[res_model].search_id(
            cr,
            SUPERUSER_ID,
            [("id", "=", values["res_id"])],
    ...
    def get_id(self, cr, uid, res_model, res_uuid, name=None):
        ...
        res_id = self.pool[res_model].search_id(
            cr, SUPERUSER_ID, [("id", "=", res_id)], context={"active_test": False}
        )
        if not res_id:
            self.write(cr, SUPERUSER_ID, uuid_id, {"active": False})
        return res_id
    ...
    def unlink_uuid(self, cr, uid, uuids, res_model=None, name=False, context=None):
        ...
        for uuid in uuids:
            ...
            if uuid_id:
                ...
                values = self.read(
                    cr, uid, uuid_id, ["res_model", "res_id"], context=context
                )
    ...
    def search_uuid(
        ...
        res_ids = model_obj.search(
            cr, uid, domain, offset=offset, limit=limit, order=order, context=context
        )
    ...
    def check_deleted(self, cr, uid, res_model):
        ...
        query = (
            ...
            " WHERE m.active=true AND m.res_model = '%s' AND r.id IS NULL "
        ) % (model_obj._table, res_model)
