...
class Execute(object):
    ...
    def execute(self, cr, uid, *args, **kwargs):
        ...
        if not args and not kwargs:
            try:
                res = self.proxy.resource.get(
                    "/execute", params_dict={"funct": repr(self.name)}
                )
        ...
        # otherwise post
            ...
            params = {"funct": repr(self.name), "_*args": None}
            ...
            try:
                res = self.proxy.resource.post(
                    "/execute", payload=payload, params_dict=params
                )
                res_str = res.body_string()
            except RequestFailed, e:
                # filter osv error
...
class RPCProxyOne(object):
    def __init__(self, url, database, user, password, obj):
        ...
        self.resource = Resource(
            "%s/rest/object/%s/%s" % (url, database, obj),
            filters=[auth],
            basic_auth_url=True,
        )

    def __getattr__(self, name):
        return lambda cr, uid, *args, **kwargs: Execute(self, name).execute(
            cr, uid, *args, **kwargs
        )
