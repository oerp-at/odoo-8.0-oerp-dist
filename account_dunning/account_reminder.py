...
class account_reminder(osv.Model):
    def send_reminder_mail(self, cr, uid, ids, context=None):
        ...
        template_id = model_data_obj.get_object_reference(
            cr, uid, "account_dunning", "email_to_customer_reminder"
        )[1]
        compose_form = model_data_obj.get_object_reference(
        ...
        if ids and template_id and compose_form:
            ...
            for reminder in self.browse(cr, uid, ids, context=context):
                ...
                if reminder_template:
                    if not profile_template_id:
                        ...
                        raise Warning(
                            _(
                                "You cannot send mass e-mails with different dunning profiles"
                            )
                        )
                ...
                if reminder.state == ("sent"):
                    raise Warning(
                        _(
                            "The reminder was already sent to the customer. If you want to send a reminder again,"
            ...
            partner_noemail_ids = partner_obj.search(
                cr,
                uid,
                [("id", "in", partner_ids), ("email", "=", False)],
                context=context,
            )
            ...
            if partner_noemail_ids:
                ...
                for partner in partner_obj.browse(
                    cr, uid, partner_noemail_ids, context=context
                ):
                    partner_names.append(partner.name)
                raise Warning(
                    _("Partner(s) %s has no email address") % ", ".join(partner_names)
                )
    ...
    _columns = {
        ...
        "profile_id": fields.many2one(
            "account.dunning_profile", "Profile", required=True
        ),
        ...
        "max_profile_line_id": fields.many2one(
            "account.dunning_profile_line", "Highest dunning level"
        ),
        "line_ids": fields.one2many(
            "account.reminder.line", "reminder_id", string="Lines"
        ),
        "state": fields.selection(
            [("validated", "Validated"), ("sent", "Reminder sent"),],
            "State",
            readonly=True,
            copy=False,
        ),
...
class account_reminder_line(osv.Model):
    ...
    _columns = {
        "reminder_id": fields.many2one(
            "account.reminder", "Reminder", ondelete="cascade", select=True
        ),
        "invoice_id": fields.many2one(
            "account.invoice", "Invoice", ondelete="cascade", select=True, required=True
        ),
        "profile_line_id": fields.many2one(
            "account.dunning_profile_line", "Dunning level", required=True
        ),
