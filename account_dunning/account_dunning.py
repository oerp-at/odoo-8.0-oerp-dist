...
class dunning_profile(osv.Model):
    ...
    _columns = {
        ...
        "line_ids": fields.one2many(
            "account.dunning_profile_line", "profile_id", "Profile lines"
        ),
        "company_id": fields.many2one("res.company", "Company", required=True),
        "shop_id": fields.many2one("sale.shop", "Shop"),
        "shop_ids": fields.many2many(
            "sale.shop",
            "account_dunning_profile_shop_rel",
            "profile_id",
            "shop_id",
            string="Shops",
        ),
        ...
        "ininvoices": fields.boolean(
            "Include Ininvoices", help="Include ininvoices on reminder"
        ),
    }
    _defaults = {
        "company_id": lambda s, cr, uid, c: s.pool.get(
            "res.company"
        )._company_default_get(cr, uid, "account.dunning_profile", context=c)
    }
...
class dunning_profile_line(osv.Model):
    def line_next(self, cr, uid, profile, profile_line, date_current, date_due):
        ...
        if delta.days > 0:
            ...
            lines = profile_line_obj.browse(
                cr,
                uid,
                profile_line_obj.search(
                    cr, uid, [("profile_id", "=", profile.id)], order="sequence asc"
                ),
            )
            for line in lines:
    ...
    _columns = {
        ...
        "sequence": fields.integer(
            "Sequence", help="It helps to order the profile lines"
        ),
        "delay": fields.integer("Days of delay"),
        "profile_id": fields.many2one(
            "account.dunning_profile", "Profile", required=True, ondelete="cascade"
        ),
        "dunning_fee_percent": fields.boolean("Dunning Fee %"),
        "dunning_fee": fields.float(
            "Dunning Fee", digits_compute=dp.get_precision("Account")
        ),
