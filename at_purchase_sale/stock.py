...
class stock_pack_operation(osv.osv):
    ...
    _columns = {
        "name": fields.function(
            _pack_description, type="char", store=False, string="Description"
        ),
        "package_count": fields.integer("Packages"),
        "package_calc": fields.function(
            _pack_calc, type="integer", store=False, string="Expected Packages"
        ),
    }
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if (
                ...
                and move.procurement_id.sale_line_id.order_id.neutral_delivery
            ):
        ...
        for proc in procs_to_check:
            pickings = list(
                set(
                    [
                        x.picking_id.id
                        for x in proc.move_ids
                        if x.picking_id and not x.picking_id.neutral_delivery
                    ]
                )
            )
            if pickings:
                pick_obj.write(
                    ...
                    uid,
                    pickings,
                    {"neutral_delivery": proc.sale_line_id.order_id.neutral_delivery},
                    context=context,
                )
        return res
...
class stock_picking(osv.osv):
    ...
    def add_package(self, cr, uid, picking_id, op_id, inc, context=None):
        ...
        if inc > 0:
            stock_operation_obj.write(
                ...
                uid,
                op.id,
                {"package_count": op.package_count + inc},
                context=context,
            )
        elif inc < 0:
            ...
            if next_package_count <= 0:
                stock_operation_obj.write(
                    cr, uid, op.id, {"package_count": 0}, context=context
                )
            else:
                stock_operation_obj.write(
                    cr,
    ...
    def process_barcode_from_ui(
        self, cr, uid, picking_id, barcode_str, visible_op_ids, context=None
    ):
        ...
        if m:
            ...
            stock_operation_obj = self.pool["stock.pack.operation"]
            op_id = stock_operation_obj.search_id(
                cr,
                uid,
                [
                    ("picking_id", "=", picking_id),
                    ("linked_move_operation_ids.move_id", "=", move_id),
                ],
                context=context,
            )
            if op_id and not visible_op_ids or op_id in visible_op_ids:
                ...
                if op.package_calc:
                    ...
                    stock_operation_obj.write(
                        ...
                        uid,
                        op_id,
                        {"package_count": op.package_count + 1},
                        context=context,
                    )
                    ...
                    # finish
                    stock_operation_obj.write(
                        cr, uid, op_id, {"qty_done": op.product_qty}, context=context
                    )
                    res["operation_id"] = op_id
                    return res
            ...
            return super(stock_picking, self).process_barcode_from_ui(
    ...
    def print_delivery(self, cr, uid, ids, context=None):
        ...
        return self.pool.get("report").get_action(
            cr, uid, ids, "stock.delivery.report", context=ctx
        )
    ...
    def print_shipping(self, cr, uid, ids, context=None):
        ...
        return self.pool.get("report").get_action(
            cr, uid, ids, "stock.delivery.label", context=ctx
        )
    ...
    def do_prepare_partial(self, cr, uid, picking_ids, context=None):
        ...
        for picking in self.browse(cr, uid, picking_ids, context=context):
            ...
            for move in picking.move_lines:
                ...
                forced_qty = (
                    (move.state == "assigned")
                    and move.product_qty - sum([x.qty for x in move_quants])
                    or 0
                )
            ...
            for vals in self._prepare_pack_ops(
                cr, uid, picking, picking_quants, forced_qties, context=context
            ):
                pack_operation_obj.create(cr, uid, vals, context=ctx)
    ...
    _columns = {
        "sender_address_id": fields.function(
            _sender_address,
