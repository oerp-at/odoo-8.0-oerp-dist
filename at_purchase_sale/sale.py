...
class sale_shop(osv.osv):
    ...
    _inherit = "sale.shop"
    _columns = {"sender_address_id": fields.many2one("res.partner", "Sender Address")}
...
class sale_order(osv.osv):
    ...
    _inherit = "sale.order"
    _columns = {
        "neutral_delivery": fields.boolean(
            "Neutral Delivery",
...
class sale_order_line(osv.osv):
    def _product_id_change(
        ...
        if not flag:
            if product_id:
                ...
                product_val = product_obj.browse(
                    cr, uid, product_id, context=product_context
                )

                # set default seller
                if product_val and product_val.seller_ids:
                    seller_line_id = product_val.seller_ids[0]
                    util.setSubDictValue(
                        res,
                        "value",
                        "supplier_id",
                        seller_line_id and seller_line_id.id or None,
                    )
    ...
    def product_id_change(
        ...
        return self._product_id_change(
            cr, uid, res, flag, product, partner_id, lang, context=context
        )
    ...
    _columns = {
        ...
        "available_supplier_ids": fields.function(
            _available_supplier_ids,
