...
class Parser(report_sxw.rml_parse):
    ...
    def _get_doc(self):
        ...
        if "purchase.order.line" == model or "purchase.order" == model:
            ...
            # reload lines
            for line in purchase_order_line_obj.browse(
                self.cr, self.uid, line_ids, context=self.localcontext
            ):
                ...
                for move in line.move_ids:
                    ...
                    # search other
                    if next_picking:
                        for cur_move in stock_move_obj.browse(
                            self.cr,
                            self.uid,
                            stock_move_obj.search(
                                self.cr, self.uid, [("purchase_line_id", "=", line.id)]
                            ),
                            self.localcontext,
                        ):
                            picking = cur_move.picking_id
            ...
            pickings = picking_obj.browse(
                self.cr, self.uid, picking_ids, self.localcontext
            )

        else:
            pickings = self.objects

        return [{"pickings": pickings, "move_ids": move_ids}]
    ...
    def _get_images(self, prep_line):
        ...
        if res is None:
            ...
            if line and purchase_line:
                ...
                for attachment in attachment_obj.browse(
                    self.cr, self.uid, ids, self.localcontext
                ):
