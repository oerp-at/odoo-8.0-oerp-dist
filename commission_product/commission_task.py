...
class commission_task(models.Model):
    ...
    def _calc_product_commission_invoice(self, taskc):
        ...
        self._recalc_product_commission_invoice(
            domain, force=True, taskc=taskc, partner=self.partner_id
        )
        ...
        for order in orders:
            ...
            if company.commission_type == "order" and analytic_account:
                for line in order.order_line:
                    if line.product_id:
                        commission_lines = commission_obj._get_product_commission(
                            line.name,
                            line.product_id,
                        ...
                        for commission_line in commission_lines:
                            ...
                            if commission:
                                if len(commission) > 1:
                                    taskc.loge(
                                        _(
                                            "Unable to update product commission! There exist more commissions of same type"
                                        ),
                                ...
                                if commission.invoice_id:
                                    taskc.loge(
                                        _(
                                            "Product commission was already invoiced and cannot be regenerated"
                                        ),
                                        ref="sale.order.line,%s"
                                        % commission_line["order_line_id"],
                                    )
    ...
    @api.model
    def _recalc_product_commission_invoice(
        self, domain, force=False, taskc=None, partner=None
    ):
        if not taskc:
            ...
            if company.commission_type == "invoice" or (
                ...
                for line in invoice.invoice_line:
                    ...
                    if analytic_account and product:
                        ...
                        order_date = (
                            orders and min([o.date_order for o in orders]) or None
                        )
