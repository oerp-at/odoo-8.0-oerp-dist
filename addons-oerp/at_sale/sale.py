...
class sale_shop(osv.osv):
    ...
    _columns = {
        ...
        "payment_default_id": fields.many2one(
            "account.payment.term", "Payment Term", required=True, select=True
        ),
        ...
        "stylesheet_id": fields.many2one(
            "report.stylesheets", "Aeroo Global Stylesheet"
        ),
        "stylesheet_landscape_id": fields.many2one(
            "report.stylesheets", "Aeroo Global Landscape Stylesheet"
        ),
        "stylesheet_intern_id": fields.many2one(
            "report.stylesheets", "Aeroo Intern Stylesheet"
        ),
        "stylesheet_intern_landscape_id": fields.many2one(
            "report.stylesheets", "Aeroo Intern Landscape Stylesheet"
        ),
    ...
    _defaults = {
        "company_id": lambda s, cr, uid, c: s.pool.get(
            "res.company"
        )._company_default_get(cr, uid, "sale.shop", context=c),
        "sequence": 10,
        "active": True,
    }
...
class sale_shop_report(osv.osv):
    ...
    _columns = {
        ...
        "shop_id": fields.many2one(
            "sale.shop", "Shop", on_delete="cascade", required=True
        ),
        "source_report_id": fields.many2one(
            "ir.actions.report.xml", "Source Report", on_delete="cascade", required=True
        ),
        "dest_report_id": fields.many2one(
        ...
        if hasattr(obj, "shop_id"):
            ...
            if shop:
                ...
                report_ids = report_obj.search(
                    ...
                    [
                        ("shop_id", "=", shop.id),
                        ("source_report_id", "=", report_xml.id),
                    ],
                )
                if report_ids:
                ...
                if styles_mode != "default":
                    ...
                    if not style_io:
                        if styles_mode == "global" or styles_mode == "intern":
                            ...
                            or styles_mode == "intern_landscape"
                        ):
...
class sale_order(osv.osv):
    def _last_invoice(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context):
            ...
            cr.execute(
                "SELECT sale_order_invoice_rel.invoice_id FROM sale_order_invoice_rel "
    ...
    def _tax_amount(self, cr, uid, id, context=None):
        ...
        for line_rec in order_rec.order_line:
            tax_calc = self.pool.get("account.tax").compute_all(
                ...
                line_rec.product_uom_qty,
                line_rec.product_id.id,
                line_rec.order_id.partner_id.id,
            )
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default.update({"client_order_ref": client_order_ref})
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        ...
        if not payment_term and shop_id:
            ...
            res["payment_term"] = (
                shop.payment_default_id and shop.payment_default_id.id or None
            )
        return res
    ...
    def onchange_partner_id(self, cr, uid, ids, part, context=None, shop_id=None):
        ...
        if value:
            ...
            if partner_invoice_id:
                delivery_onchange = self.onchange_invoice_partner_id(
                    cr,
                    uid,
                    ids,
                    False,
    ...
    def onchange_invoice_partner_id(
        self,
        cr,
        uid,
        ids,
        ...
        fiscal_position = self.pool["account.fiscal.position"].get_fiscal_position(
            cr, uid, company_id, partner_id, invoice_partner_id, context=context
        )
        if fiscal_position:
        ...
        if real_delay:
            if company.calendar_id:
                date_planned = self.pool.get("resource.calendar").passed_range(
                    cr, uid, company.calendar_id.id, start_date, real_delay
                )["to"]
            else:
    ...
    def _calc_discount(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context):
            ...
            if not discount_amount:
                res[obj.id] = {"discount": 0.0, "discount_amount": 0.0}
            else:
                amount_nodisc = obj.amount_untaxed + discount_amount
                discount = 100.0 / amount_nodisc * discount_amount
                res[obj.id] = {"discount": discount, "discount_amount": discount_amount}
        return res
    ...
    _columns = {
        ...
        "last_invoice_id": fields.function(
            _last_invoice,
            string="Last Invoice",
            readonly=True,
            type="many2one",
...
class sale_order_line(osv.osv):
    def _price_unit_norecalc(self, cr, uid, ids, product_id, context=None):
        if product_id:
            prod_vals = self.pool["product.product"].read(
                cr, uid, product_id, ["list_price"], context=context
            )
    ...
    def _line_sum(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids):
            ...
            tax_id = line.tax_id
            for tax in self.pool.get("account.tax").compute_all(
                cr,
                uid,
                tax_id,
    ...
    _columns = {
        ...
        "price_nocalc": fields.boolean(
            "No Price Calculation", readonly=True, copy=False
        ),
        "price_subtotal_nodisc": fields.function(
            _amount_line_nodisc,
            string="Subtotal w/o Disc.",
            digits_compute=dp.get_precision("Sale Price"),
            multi="_amount_line_nodisc",
        ),
        "discount_amount": fields.function(
            _amount_line_nodisc,
            string="Discount",
