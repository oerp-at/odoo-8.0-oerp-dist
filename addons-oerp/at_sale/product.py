...
class product_product(osv.osv):
    def onchange_brutto_price(
        self, cr, uid, ids, brutto_price, taxes_id, company_id, context=None
    ):
        return self.pool["product.template"]._onchange_price(
            cr,
            uid,
            ids,
            brutto_price,
...
class product_template(osv.osv):
    def _onchange_price(
        ...
        if not company_id:
            company_id = company_obj._company_default_get(
                cr, uid, "product.template", context=context
            )
        ...
        if taxes_list:
            ...
            if taxes:
                taxes = [
                    t
                    for t in taxes
                    if not t.company_id or t.company_id.id == company_id
                ]
        ...
        if taxes:
            price_all = None
            if tax_included:
                price_all = tax_obj.compute_full(
                    cr, uid, taxes, price, 1, force_included=True
                )
            else:
                price_all = tax_obj.compute_full(
                    cr, uid, taxes, price, 1, force_excluded=True
                )

            brutto = price_all["total_included"]
            netto = price_all["total"]
            price = tax_obj.compute_full(cr, uid, taxes, netto, 1, netto_to_price=True)[
                "total_included"
            ]
    ...
    def onchange_brutto_price(
        ...
        return self._onchange_price(
            cr, uid, ids, brutto_price, taxes_id, company_id, True, context=context
        )
    ...
    def onchange_netto_price(
        ...
        return self._onchange_price(
            cr, uid, ids, brutto_price, taxes_id, company_id, False, context=context
        )
    ...
    def _calculate_price(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for product in self.browse(cr, SUPERUSER_ID, ids, context=context):
            ...
            res[product.id] = {"brutto_price": brutto, "netto_price": netto}
        return res

    _inherit = "product.template"
    _columns = {
        "brutto_price": fields.function(
            _calculate_price,
            type="float",
            string="Brutto Price",
            help="The brutto price is based on the given tax.",
            multi="_calculate_price",
        ),
        "netto_price": fields.function(
            _calculate_price,
            type="float",
            string="Netto Price",
            help="The netto price without tax.",
            multi="_calculate_price",
        ),
    }
