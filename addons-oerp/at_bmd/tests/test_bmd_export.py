...
class TestBmdExport(TransactionCase):
    ...
    def test_bmd_export_bmd55(self):
        ...
        buerf = self.env["bmd.export.file"].search(
            [("bmd_export_id", "=", export.id), ("export_name", "=", "buerf")], limit=1
        )

        self.assertTrue(buerf, "Check if buerf file was created")

        stamerf = self.env["bmd.export.file"].search(
            [("bmd_export_id", "=", export.id), ("export_name", "=", "stamerf")],
            limit=1,
        )
