...
class crm_case_categ(osv.osv):
    ...
    _columns = {"code": fields.char("Code", size=8, select=True)}
...
class crm_lead(osv.osv):
    ...
    def action_schedule_meeting(self, cr, uid, ids, context=None):
        ...
        user = lead.user_id or self.pool["res.users"].browse(
        ...
        meeting_context = {
            "search_default_opportunity_id": lead.type == "opportunity"
            and lead.id
            or False,
    ...
    _columns = {"editor_id": fields.many2one("res.users", "Editor", select=True)}
    _defaults = {
