...
class procurement_order(osv.Model):
    ...
    def _convert_qty_company_hours(self, cr, uid, procurement, context=None):
        ...
        if sale_line:
            self.pool["sale.order.line"]._convert_qty_company_hours(
                cr, uid, sale_line, context=context
            )
        else:
            # if no sale line
            ...
            return super(procurement_order, self)._convert_qty_company_hours(
                cr, uid, procurement, context=context
            )

    def _is_procurement_task(self, cr, uid, procurement, context=None):
        is_procurement_task = super(procurement_order, self)._is_procurement_task(
            cr, uid, procurement, context=context
        )
    ...
    def _create_service_task(self, cr, uid, procurement, context=None):
        ...
        if sale_line:
            task_update = sale_line_obj._prepare_task(
                cr, uid, sale_line, context=context
            )
            ...
            if task:
                ...
                self.write(
                    cr, uid, [procurement.id], {"task_id": task.id}, context=context
                )
        ...
        task_id = super(procurement_order, self)._create_service_task(
            cr, uid, procurement, context=context
        )
