...
class sale_order(osv.osv):
    def onchange_shop_id(self, cr, uid, ids, shop_id, state, project_id, context=None):
        ...
        if shop_id:
            ...
            if not categories:
                res["value"]["shop_category_ids"] = self.pool[
                    "product.category"
                ].search(cr, uid, [("parent_id", "=", False)])
            else:
    ...
    def _shop_category_ids(self, cr, uid, ids, field_name, args, context=None):
        ...
        for order in self.browse(cr, uid, ids, context):
            ...
            if not categories:
                res[order.id] = category_obj.search(
    ...
    _inherit = "sale.order"
    _columns = {
        "shop_category_ids": fields.function(
            _shop_category_ids,
