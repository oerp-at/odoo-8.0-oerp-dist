...
class stock_move(osv.osv):
    def _update_expected_date(self, cr, uid, moves, context=None):
        ...
        for move in moves:
            ...
            while move_dest:
                orig_expected_date = self.read(
                    cr, uid, orig_move.id, ["date_expected"], context
                )["date_expected"]
                dest_expected_date = self.read(
                    cr, uid, move_dest.id, ["date_expected"], context
                )["date_expected"]
                expected_date = util.mergeTimeStr(
                    util.strToTime(orig_expected_date) + relativedelta(days=delay),
                    dest_expected_date,
                )
                #
                if dest_expected_date != expected_date:
                    ...
                    if move_dest.picking_id:
                        picking_obj.write(
                            cr,
                            uid,
                            move_dest.picking_id.id,
                            {"move_lines": [(1, move_dest.id, vals)]},
                            context,
                        )
                    else:
                ...
                delay = (
                    orig_move.location_id and orig_move.location_id.chained_delay or 0
                )
