...
class delivery_carrier(osv.Model):
    ...
    _columns = {"api": fields.selection(_api_select, string="API")}
...
class delivery_order(osv.Model):
        ...
        if vals.get("name", "/") == "/":
            vals["name"] = (
                self.pool.get("ir.sequence").get(
                    cr, uid, "delivery.order", context=context
                )
                or "/"
            )
        ...
        for order_id in ids:
            ...
            picking_ids = picking_obj.search(
                ...
                [
                    ("state", "=", "done"),
                    ("carrier_id", "!=", False),
                    ("carrier_order_id", "=", False),
                    ("carrier_status", "!=", False),
                ],
                context=context,
            )
            picking_obj.write(
                cr, uid, picking_ids, {"carrier_order_id": order_id}, context=context
            )
        return True
    ...
    _columns = {
        ...
        "user_id": fields.many2one(
            "res.users", "User", ondelete="restrict", required=True
        ),
