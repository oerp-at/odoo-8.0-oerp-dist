            ...
            for picking in order.picking_ids:
                if (
                    ...
                    for move in picking.move_lines:
                        productionEntries = production_obj.search_read(
                            cr,
                            uid,
                            [
                                ("move_prod_id", "=", move.id),
                        ...
                        if productionEntries:
                            for prodEntry in productionEntries:
                                ...
                                if prodEntry["state"] == "confirmed":
                                    production_obj.action_assign(
                                        cr, uid, production_ids, context=context
                                    )
                                    prodEntry["state"] = production_obj.read(
                                        cr,
                                        uid,
                                        production_id,
                                        ["state"],
                                ...
                                if prodEntry["state"] == "ready":
                                    ...
                                    values = produce_obj.default_get(
                                        cr,
                                        uid,
                                        [
                                            "product_qty",
                                    ...
                                    values.update(
                                        produce_obj.on_change_qty(
                                            cr,
                                            uid,
                                            [],
                                            values.get("product_qty", 0.0),
                                            [],
                                            context=prodContext,
                                        )["value"]
                                    )
                                    produce_id = produce_obj.create(
                                        cr, uid, values, context=prodContext
                                    )
                                    produce_obj.do_produce(
                                        cr, uid, [produce_id], context=prodContext
                                    )
                                    produce_obj.unlink(
                                        cr, uid, produce_id, context=prodContext
                                    )
