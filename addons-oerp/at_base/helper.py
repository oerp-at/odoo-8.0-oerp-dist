...
def getRangeName(cr, uid, start_date, end_date, context):
    ...
    if start_date == end_date:
        ...
        return "%s, %s" % (
            f.formatLang(start_date, date=True),
            f.getDayShortName(start_date),
        )
    else:
        ...
        if start.month == 1 and end.month == 3 and start.year == end.year:
            ...
            if (
                util.getFirstOfMonth(start_date) == start_date
                and util.getEndOfMonth(end_date) == end_date
            ):
                ...
                return "%s - %s" % (
                    f.formatLang(start_date, date=True),
                    f.formatLang(end_date, date=True),
                )
...
def strToLocalTimeStr(cr, uid, str_time, context):
    ...
    converted = datetime_field.context_timestamp(
        cr, uid, timestamp=timestamp, context=context
    )
...
def onChangeValuesPool(
    cr, uid, line_obj, values, onchange_values, force=False, context=None
):
    if onchange_values:
        update_values = onchange_values.get("value")
        if update_values:
