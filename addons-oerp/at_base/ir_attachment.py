...
class ir_attachment(osv.Model):
    ...
    def _save_file(self, cr, uid, oid, values, context=None):
        ...
        # update file
        if oid:
            fname_to_delete = self.read(cr, uid, oid, ["store_fname"], context=context)[
                "store_fname"
            ]
        ...
        cr.execute(
            "UPDATE ir_attachment SET file_size=%s WHERE id=%s", (file_size, oid)
        )
        return oid

    _name = "ir.attachment"
    _inherit = "ir.attachment"
    _columns = {"origin": fields.char("Origin")}
