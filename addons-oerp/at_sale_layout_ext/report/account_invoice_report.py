...
class Parser(extreport.basic_parser):
    ...
    def _taxes(self, invoice):
        ...
        for t_tax_id, t_tax_amount in t_invoice_obj._tax_amount(
            self.cr, self.uid, invoice.id, self.localcontext
        ).items():
    ...
    def _sale_layout(self, invoice):
        ...
        res = invoice_obj.sale_layout_lines(
            self.cr,
            self.uid,
            [invoice.id],
        ...
        for categ in res:
            ...
            if lines:
                for line in lines:
                    ...
                    line_res = {}
                    line_res["tax_id"] = (
                        ", ".join(map(lambda x: x.name, line.invoice_line_tax_id)) or ""
                    )
