...
class Parser(extreport.basic_parser):
    ...
    def _currency(self, sale_order):
        return (
            ...
            and sale_order.pricelist_id.currency_id
            and sale_order.pricelist_id.currency_id.symbol
        ) or ""
    ...
    def _taxes(self, sale_order):
        ...
        for t_tax_id, t_tax_amount in t_sale_obj._tax_amount(
            self.cr, self.uid, sale_order.id, self.localcontext
        ).items():
    ...
    def _sale_layout(self, sale_order):
        ...
        res = sale_obj.sale_layout_lines(
            self.cr,
            self.uid,
            [sale_order.id],
        ...
        for categ in res:
            ...
            if lines:
                for line in lines:
                    ...
                    product_categ_values = product_categ_dict.get(
                        product_categ_name, None
                    )
                    if product_categ_values is None:
                        pos = 1
                        product_categ_values = {
                            "prod_categ": product_categ,
                    ...
                    line_res = {}
                    line_res["tax_id"] = (
                        ", ".join(map(lambda x: x.name, line.tax_id)) or ""
                    )
                    line_res["name"] = line_name
                    line_res["note"] = line_note
                    line_res["product_uom_qty"] = (
                        line.product_uos
                        and line.product_uos_qty
                        or line.product_uom_qty
                        or 0.00
                    )
                    line_res["product_uom"] = (
                        line.product_uos
                        and line.product_uos.name
                    ...
                    line_res["pos"] = "{:02d}.{:02d}.{:03d}".format(
                        categ_pos, product_categ_values["sequence"], pos
                    )
                    line_res["id"] = line.id
                    line_res["line"] = line
