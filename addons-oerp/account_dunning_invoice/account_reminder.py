...
class account_reminder(osv.Model):
    def send_reminder_mail(self, cr, uid, ids, context=None):
        res = super(account_reminder, self).send_reminder_mail(
        ...
        for reminder in self.browse(cr, uid, ids, context=context):
            for line in reminder.line_ids:
                ...
                report.create(
                    cr, uid, [invoice.id], {"model": "account.invoice"}, report_ctx
                )

                att_id = report_obj.get_attachment_id(
                    cr, uid, "account.invoice", invoice.id, report_name, context=context
                )
                if att_id:
