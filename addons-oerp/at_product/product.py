    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for record in reads:
            ...
            if record["parent_id"] and (
                not context or context.get("show_parent", True)
            ):
...
class product_product(osv.osv):
    def _product_overview(self, cr, uid, ids, context=None):
        ...
        category_names = dict(
            self.pool["product.category"].name_get(
        ...
        for product in products:
            ...
            if ntemplate is None:
                ntemplate = {"name": pt.name, "products": []}
    ...
    def _pricelist_view(self, cr, uid, version, context=None):
        ...
        def add_product(product):
            if not product.id in product_map:
                product_vals = {"product": product, "price": product.list_price}
                # set to product map
                ...
                if not category_vals:
                    category_vals = {"name": product.categ_id.name, "products": []}
        ...
        price_dict = pricelist_obj._price_get_multi(
            cr, uid, version, products_by_qty_by_partner, context
        )
...
class product_supplierinfo(osv.osv):
    ...
    _columns = {"cost_price": fields.float("Cost price")}
