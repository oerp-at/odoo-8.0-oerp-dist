...
class commission_invoice_wizard(osv.osv_memory):
    def _name_default(self, cr, uid, context=None):
        ...
        for line in commission_line_obj.browse(
            cr, uid, util.active_ids(context, commission_line_obj), context=context
        ):
        ...
        return _("Provision %s") % (
            helper.getRangeName(cr, uid, date_min, date_max, context),
        )
    ...
    def _inv_line_note_get(self, cr, uid, wizard, analytic_line, context=None):
        ...
        if analytic_line.base_commission == analytic_line.total_commission:
            if analytic_line.val_based:
                return None
            return _("%s %% Commission from %s %s") % (
                f.formatLang(analytic_line.base_commission),
                f.formatLang(analytic_line.price_sub, dp="Sale Price"),
                cur,
            )

        if analytic_line.base_commission < analytic_line.total_commission:
            return _("%s %% Commission +%s %% Bonus from %s %s") % (
                f.formatLang(analytic_line.base_commission),
            ...
            return _("%s %% Commission -%s %% Malus from %s %s") % (
                f.formatLang(analytic_line.base_commission),
    ...
    def do_create(self, cr, uid, ids, context=None):
        ...
        line_ids = commission_line_obj.search(
            cr,
            uid,
            [("id", "in", util.active_ids(context))],
            order="partner_id, date asc",
        )
        for line in commission_line_obj.browse(cr, uid, line_ids):
            ...
            if not partner or partner.id != line.partner_id.id:
                ...
                if team:
                    shop_id = shop_obj.search_id(
                        cr, uid, [("team_id", "=", team.id)], context=invContext
                    )
                    if shop_id:
                ...
                helper.onChangeValuesPool(
                    ...
                    invoice_obj,
                    inv_values,
                    invoice_obj.onchange_company_id(
                        cr,
                        uid,
                        [],
                        company.id,
                ...
                helper.onChangeValuesPool(
                    ...
                    invoice_obj,
                    inv_values,
                    invoice_obj.onchange_journal_id(
                        cr, uid, [], inv_values["journal_id"], context=invContext
                    ),
                    context=context,
                )
            ...
            if detail and line.sale_partner_id:
                ...
                invoice_line_id = invoice_line_obj.create(
                    cr, uid, values, context=invContext
                )
                commission_line_obj.write(
                    cr,
                    uid,
                    line.id,
                    {
                        "invoiced_id": invoice_id,
        ...
        if invoice_ids:
            invoice_obj.button_compute(
                cr, uid, invoice_ids, set_total=True, context=context
            )
            ...
            mod_ids = mod_obj.search(
                cr, uid, [("name", "=", view_id)], context=context
            )[0]
            res_id = mod_obj.read(cr, uid, mod_ids, ["res_id"], context=context)[
                "res_id"
            ]
    ...
    _columns = {
        ...
        "detail_ref": fields.boolean(
            "Detailed Ref.", help="Detailed Reference is displayed"
        ),
    }
    _defaults = {"name": _name_default, "detail_ref": True}
