...
class account_invoice(osv.osv):
    def _replace_invoice_ids_with_id(self, cr, uid, ids, oid, context=None):
        super(account_invoice, self)._replace_invoice_ids_with_id(
            cr, uid, ids, oid, context
        )
    ...
    def action_cancel(self, cr, uid, ids, *args, **kwargs):
        commission_line_obj = self.pool.get("commission.line")
        commission_line_ids = commission_line_obj.search(
            cr, uid, [("invoice_line_id.invoice_id", "in", ids)]
        )
