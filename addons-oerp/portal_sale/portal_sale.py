...
class sale_order(osv.Model):
    ...
    _columns = {
        "portal_payment_options": fields.function(
            _payment_block_proxy, type="html", string="Portal Payment Options"
        ),
    }
        ...
        for this in self.browse(cr, SUPERUSER_ID, ids, context=context):
            if this.state not in ("draft", "cancel") and not this.invoiced:
                result[this.id] = payment_acquirer.render_payment_block(
                    ...
                    this.name,
                    this.amount_total,
                    this.pricelist_id.currency_id.id,
    ...
    def action_quotation_send(self, cr, uid, ids, context=None):
        ...
        try:
            template_id = self.pool.get("ir.model.data").get_object_reference(
                cr, uid, "portal_sale", "email_template_edi_sale"
            )[1]
    ...
    def get_formview_action(self, cr, uid, id, context=None):
        ...
        if user.share:
            ...
            action_xmlid = (
                "action_quotations_portal"
                if document.state in ("draft", "sent")
                else "action_orders_portal"
            )
            return self.pool["ir.actions.act_window"].for_xml_id(
                cr, uid, "portal_sale", action_xmlid, context=context
            )
...
class account_invoice(osv.Model):
    ...
    def _portal_payment_block(self, cr, uid, ids, fieldname, arg, context=None):
        ...
        for this in self.browse(cr, uid, ids, context=context):
            if (
                ...
                and this.state not in ("draft", "done")
    ...
    def action_invoice_sent(self, cr, uid, ids, context=None):
        """  Override to use a modified template that includes a portal signup link """
        action_dict = super(account_invoice, self).action_invoice_sent(
            cr, uid, ids, context=context
        )
        try:
            template_id = self.pool.get("ir.model.data").get_object_reference(
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            ...
            if partner not in invoice.message_follower_ids:
                self.message_subscribe(
                    cr, uid, [invoice.id], [partner.id], context=context
                )
    ...
    def get_formview_action(self, cr, uid, id, context=None):
        ...
        if user.share:
            return self.pool["ir.actions.act_window"].for_xml_id(
                cr, uid, "portal_sale", "portal_action_invoices", context=context
            )
        return super(account_invoice, self).get_formview_action(
    ...
    def _postprocess_sent_message(self, cr, uid, mail, context=None, mail_sent=True):
        if mail_sent and mail.model == "sale.order":
            ...
            if partner not in order.message_follower_ids:
                so_obj.message_subscribe(
                    cr, uid, [mail.res_id], [partner.id], context=context
                )
            # Add all recipients of the email as followers
            for p in mail.partner_ids:
                if p not in order.message_follower_ids:
                    so_obj.message_subscribe(
                        cr, uid, [mail.res_id], [p.id], context=context
                    )
        return super(mail_mail, self)._postprocess_sent_message(
            cr, uid, mail=mail, context=context, mail_sent=mail_sent
        )
