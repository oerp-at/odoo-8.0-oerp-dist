                ...
                if task:
                    if task.project_id.id != project.id:
                        tasks = task_obj.search(
                            [("project_id", "=", project.id), ("name", "=", task.name)],
                            limit=1,
                        )
                        if not tasks:
                            tasks = task_obj.search(
    ...
    def action_change(self):
        for wizard in self:
            ...
            if line.project_work_id:
                ...
                work = (
                    self.env["project.task.work"]
                    .with_context(no_analytic_entry=True)
                    .create(work_values)
                )
                # write for update
            ...
            wizard.task_id.write(
                {
                    "remaining_hours": wizard.task_id.planned_hours
                    - wizard.task_id.effective_hours
                }
            )
            if old_task and old_task.id != wizard.task_id.id:
                old_task.write(
                    {
                        "remaining_hours": old_task.planned_hours
                        - old_task.effective_hours
                    }
                )
