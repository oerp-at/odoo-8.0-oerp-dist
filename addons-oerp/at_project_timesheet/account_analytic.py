...
class account_analytic_line(osv.Model):
    def _project_task_id(self, cr, uid, ids, field_names, arg, context=None):
        ...
        # get tasks
        cr.execute(
            "SELECT ts.line_id, w.task_id, w.id FROM project_task_work w "
            " INNER JOIN hr_analytic_timesheet ts ON ts.id = w.hr_analytic_timesheet_id "
            " WHERE ts.line_id IN %s AND w.task_id IS NOT NULL",
            (tuple(ids),),
        )
        ...
        for line in self.browse(cr, SUPERUSER_ID, ids, context=context):
            ...
            if line.to_invoice:
                ...
                if account.is_contract:
                    if (
                        account.recurring_invoices
                        and account.recurring_invoice_line_ids
                    ):
                        res[line.id]["will_invoiced"] = True
    ...
    _columns = {
        "project_task_id": fields.function(
            _project_task_id,
            type="many2one",
            obj="project.task",
            string="Task",
            radonly=True,
            multi="_project_task_id",
        ),
        "project_work_id": fields.function(
            _project_task_id,
            type="many2one",
        ...
        "is_project_task": fields.function(
            _project_task_id,
            type="boolean",
            string="Task",
            radonly=True,
        ...
        "will_invoiced": fields.function(
            _project_task_id,
            type="boolean",
            string="Will be invoiced",
            radonly=True,
            multi="_project_task_id",
        ),
    }
