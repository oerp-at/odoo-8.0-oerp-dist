...
class procurement_order(osv.Model):
    def _get_product_supplier(self, cr, uid, procurement, context=None):
        ...
        if not res:
            return super(procurement_order, self)._get_product_supplier(
    ...
    def make_po(self, cr, uid, ids, context=None):
        ...
        for procurement in self.browse(cr, uid, ids, context=context):
            ...
            if not partner:
                ...
                name = (
                    seq_obj.get(cr, uid, "purchase.order")
                ...
                po_vals = {
                    ...
                    "fiscal_position": po_obj.onchange_partner_id(
                        cr, uid, None, partner.id, context=context
                ...
                po_id = self.create_procurement_purchase_order(
                    cr, SUPERUSER_ID, procurement, po_vals, line_vals, context=context
                )
                po_line_id = (
    ...
    def create_procurement_purchase_order(
        self, cr, uid, procurement, po_vals, line_vals, context=None
    ):
            ...
            if product:
                ...
                if purchase_line_ids:
                    purchase_line_obj.write(
                        cr, uid, purchase_line_ids, line_vals, context=context
                    )

                # cancel or remove unselected
                unused_line_ids = purchase_line_obj.search(
                    cr,
                    uid,
                    [
                        ("sale_line_id", "=", sale_order_line.id),
                        ("partner_id", "!=", sale_order_line.supplier_id.id),
                        ("product_id", "=", product.id),
                    ],
                    context=context,
                )
                if unused_line_ids:
                    for line in purchase_line_obj.browse(
                        cr, uid, unused_line_ids, context=context
                    ):
                ...
                if purchase_line_id:
                    ...
                    self.pool["purchase.order"].write(
                        cr, uid, order.id, po_vals, context=context
                    )
                    return order.id
