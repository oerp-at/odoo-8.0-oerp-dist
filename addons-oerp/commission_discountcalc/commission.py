...
class commission_line(osv.Model):
    ...
    def _validate_sale_commission(
        ...
        commission = super(commission_line, self)._validate_sale_commission(
            cr, uid, values, obj=obj, context=context
        )
        if company:
            rule = company.cdisc_rule
            if rule and not company.cdisc_date or company.cdisc_date <= values["date"]:
                if rule == "mhalf":
                    commission = self._cdisc_rule_mhalf(
                        cr, uid, values, obj=obj, company=company, context=context
                    )
                    # commission = getattr(self, rule)(cr, uid, values, obj=obj, context=context)

        return commission

    def _cdisc_rule_mhalf(
        self, cr, uid, commission, obj=None, company=None, context=None
    ):
        if (
            commission.get("val_based")
            or not obj
            or obj._model._name != "sale.order.line"
            or not obj.discount
            or obj.discount_action
        ):
            return commission
        ...
        if percent:
            ...
            if obj.discount >= 100.0:
                ...
                # deduct discount
                taxes = self.pool["account.tax"].compute_all(
                    cr,
                    uid,
                    obj.tax_id,
                    obj.price_unit,
