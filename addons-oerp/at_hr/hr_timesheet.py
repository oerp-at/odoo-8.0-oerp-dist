...
class hr_timesheet(osv.osv):
    ...
    def _get_date_to(self, cr, uid, str_date, context=None):
        ...
        r = user.company_id and user.company_id.timesheet_range or "month"
        if r == "month":
            return (d_date + relativedelta(months=+1, day=1, days=-1)).strftime(
                "%Y-%m-%d"
            )
        elif r == "week":
    ...
    def get_timesheet_day(self, cr, uid, str_date, context=None):
        ...
        if employee_ids:
            ...
            if timesheet_ids:
                ...
                ts_day_ids = ts_day_obj.search(
                    cr,
                    uid,
                    [("name", "=", str_date), ("sheet_id", "=", timesheet_ids[0])],
                )
                if ts_day_ids:
    ...
    def get_timesheet_lazy(self, cr, uid, str_date, context=None):
        ...
        if not len(employee_ids):
            ...
            raise osv.except_osv(
                _("Error !"), _("More than one employee defined for your user !")
            )
        ...
        if len(timesheet_ids) > 1:
            raise osv.except_osv(
                _("Error !"),
                _(
                    "More than one timesheet for the same user and the same time range !"
                ),
            )
            ...
            vals = {
                ...
                "company_id": self.pool.get("res.company")._company_default_get(
                    cr, uid, "hr_timesheet_sheet.sheet", context=context
                ),
            }
    ...
    def get_timesheet_data(self, cr, uid, oid, context=None):
        ...
        for key, value in days.items():
            value["lines"] = line_obj.search(
                cr,
                uid,
                [("date", "=", key), ("sheet_id", "=", timesheet.id)],
                order="id asc",
            )
...
class hr_timesheet_line(osv.osv):
    def _amount_brutto(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if product:
                amount_all = tax_obj.compute_all(
                    cr,
                    uid,
    ...
    _columns = {
        "amount_brutto": fields.function(
            _amount_brutto,
            string="Amount (Brutto)",
            type="float",
