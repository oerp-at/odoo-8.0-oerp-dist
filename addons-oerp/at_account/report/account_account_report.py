...
class Parser(extreport.basic_parser):
    ...
    def _accounts_get(self):
        ...
        for account in account_ids:
            ...
            res_value["debit_from"] = account_obj._compute_account(
                self.cr,
                self.uid,
                [account.id],
                date_till=self.localcontext["date_from"],
            )[account.id]["debit"]
            res_value["credit_from"] = account_obj._compute_account(
                self.cr,
                self.uid,
                [account.id],
                date_till=self.localcontext["date_from"],
            )[account.id]["credit"]
            res_value["balance_from"] = account_obj._compute_account(
                self.cr,
                self.uid,
                [account.id],
                date_till=self.localcontext["date_from"],
            ...
            res_value["debit_to"] = account_obj._compute_account(
                self.cr, self.uid, [account.id], date_till=self.localcontext["date_to"]
            )[account.id]["debit"]
            res_value["credit_to"] = account_obj._compute_account(
                self.cr, self.uid, [account.id], date_till=self.localcontext["date_to"]
            )[account.id]["credit"]
            res_value["balance_to"] = account_obj._compute_account(
                self.cr, self.uid, [account.id], date_till=self.localcontext["date_to"]
            )[account.id]["balance"]
