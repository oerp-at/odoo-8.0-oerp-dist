...
class inovice_attachment_wizard(osv.TransientModel):
    ...
    def action_import(self, cr, uid, ids, context=None):
        ...
        if not report_obj.write_attachment(
            cr,
            uid,
            "account.invoice",
            invoice_id,
            ...
            context=context,
            origin="account.invoice.attachment.wizard",
        ):
            raise osv.except_osv(
                _("Error!"),
                _("Unable to import document (check if invoice is validated)"),
            )
        return {"type": "ir.actions.act_window_close"}

    _columns = {"document": fields.binary("Document")}
