...
class res_users(osv.osv):
    ...
    _columns = {
        "password_crypt2": fields.char(
            string="Encrypted Password", invisible=True, copy=False
        )
    }

    def init(self, cr):
        cr.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_name='res_users' and column_name='password_crypt'"
        )
        cr_res = cr.fetchone()
    ...
    def check_credentials(self, cr, uid, password):
        ...
        cr.execute(
            "SELECT password, password_crypt2 FROM res_users WHERE id=%s AND active",
            (uid,),
        )
        encrypted = None
        if cr.rowcount:
        ...
        try:
            ...
            if encrypted:
                ...
                if valid_pass:
                    self.write(
                        cr, uid, uid, {"password": password, "password_crypt2": None}
                    )
                    return
