...
class portal_download(models.Model):
    ...
    def _compute_download_link(self):
        self.download_link = (
            '<a href="/portal_download/start/%s">%s</a>' % self.name_get()[0]
        )
    ...
    code = fields.Char(
        "Code", help="Download code which can be used for identifying or as shortcut"
    )
    permission_ids = fields.One2many(
        "portal.download.perm", "download_id", "Permissions"
    )
...
class portal_download_perm(models.Model):
    ...
    download_id = fields.Many2one(
        "portal.download", "Download", required=True, ondelete="cascade"
    )
    download_count = fields.Integer("Download Count", readonly=True, copy=False)
    download_key = fields.Char(
        "Download Key", compute="_download_key", store=False, readonly=True
    )
    download_key_intern = fields.Char("Download Key (Intern)", copy=False)
    download_link = fields.Char(
        "Download Link", compute="_download_link", readonly=True
    )
    ...
    def _download_link(self):
        ...
        if download_key:
            url = "/portal_download/start/%s?key=%s" % (
                self.download_id.id,
                download_key,
            )
            baseurl = self.env["ir.config_parameter"].get_param(
    ...
    def _download_key(self):
        ...
        if not download_key and self.id:
            ...
            self._cr.execute(
                "UPDATE portal_download_perm SET download_key_intern=%s WHERE id=%s",
                (download_key, self.id),
            )
