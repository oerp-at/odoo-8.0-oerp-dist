...
class mail_compose_message(osv.TransientModel):
    def _get_render_env(
        ...
        variables = super(mail_compose_message, self)._get_render_env(
    ...
    def _res_ids(self, wizard, context=None):
        ...
        if mass_mode and wizard.use_active_domain and wizard.model:
            res_ids = self.pool[wizard.model].search(
                cr, uid, eval(wizard.active_domain), context=context
            )
    ...
    def onchange_template_id(
        self, cr, uid, ids, template_id, composition_mode, model, res_id, context=None
    ):
        res = super(mail_compose_message, self).onchange_template_id(
            cr, uid, ids, template_id, composition_mode, model, res_id, context=context
        )
        if context:
