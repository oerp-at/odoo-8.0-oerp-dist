...
class hr_employee(osv.osv):
    def _get_contract(
        self, cr, uid, employee_id, date_from=None, date_to=None, context=None
    ):
        if not date_from:
    ...
    def attendance_action_change(self, cr, uid, ids, context=None):
                ...
                if contract:
                    ...
                    if break_interval and break_duration and contract.break_auto:
                        attendance_ids = attendance_obj.search(
                            cr, uid, [("employee_id", "=", employee.id)], limit=1
                        )
                        if attendance_ids:
                            ...
                            until_time = util.strToTime(
                                context.get(
                                    "default_name", time.strftime(util.DHM_FORMAT)
                                )
                            )
        ...
        att_id = super(hr_employee, self).attendance_action_change(
