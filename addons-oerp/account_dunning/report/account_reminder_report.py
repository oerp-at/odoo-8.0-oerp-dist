...
class Parser(extreport.basic_parser):
    def __init__(self, cr, uid, name, context=None):
        ...
        self.localcontext.update({"util": util, "remind": self._remind})
    ...
    def _eval_text(self, text, partner, profile, str_date):
        ...
        if text:
            val = {
                ...
                "user_signature": self.pool.get("res.users")
                .browse(self.cr, self.uid, self.uid, self.localcontext)
                .signature,
            }
            res = text % val
        return res
    ...
    def _remind(self, remind):
        ...
        text_before = self._eval_text(
            profile_line.description, partner, profile, dunning_date
        )
        text_after = self._eval_text(
            profile_line.description2, partner, profile, dunning_date
        )
        ...
        for line in remind.line_ids:
            ...
            lines.append({"amount": invoice.residual, "inv": invoice, "obj": line})
            ...
            for invoice in invoice_obj.browse(
                ...
                if invoice.residual:
                    ...
                    if invoice.type in ("out_refund", "in_invoice"):
                        ...
                        invoice_amount = invoice.residual
                    invoices.append({"inv": invoice, "amount": invoice_amount})
