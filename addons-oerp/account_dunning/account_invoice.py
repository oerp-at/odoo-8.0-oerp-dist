...
class account_invoice(osv.osv):
    ...
    def _get_followup_line(self, cr, uid, ids, field_name, args, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context):
            reminder_line_ids = reminder_line_obj.search(
                cr, uid, [("invoice_id", "=", invoice.id)]
            )
            if reminder_line_ids:
                reminder_line = reminder_line_obj.browse(
                    cr, uid, reminder_line_ids[0], context
                )
    ...
    _columns = {
        "profile_line_id": fields.function(
            _get_followup_line,
            type="many2one",
            obj="account.dunning_profile_line",
            method=True,
