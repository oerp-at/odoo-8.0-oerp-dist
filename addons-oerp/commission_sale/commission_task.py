...
class commission_task(models.Model):
    ...
    def _commission_sale(self, order, context=None):
        ...
        for line in order.order_line:
            ...
            if product:
                commission_lines = commission_obj._get_sale_commission(
                    line.name,
                    order.user_id,
                    order.partner_id,
                    product,
            ...
            if company.commission_type == "order" and analytic_account:
                ...
                for commission_line in commission_lines:
                    ...
                    commission = commission_obj.search(
                        [
                            ("order_id", "=", commission_line["order_id"]),
                    ...
                    if commission:
                        if len(commission) > 1:
                            taskc.loge(
                                _(
                                    "Unable to update commission! There exist more commissions of same type"
                                ),
                                ref="sale.order.line,%s"
                                % commission_line["order_line_id"],
                            )
                            continue
                        if commission.invoice_id:
                            taskc.loge(
                                _(
                                    "Commission was already invoiced and cannot be regenerated"
                                ),
                                ref="sale.order.line,%s"
                                % commission_line["order_line_id"],
                            )
    ...
    def _recalc_sale_commission_invoice(self, domain, force=False, taskc=None):
        ...
        for invoice in invoices:
            ...
            if company.commission_type == "invoice" or (
                force and invoice.type in ("out_refund", "in_invoice")
            ):
                ...
                for line in invoice.invoice_line:
                    ...
                    if product:
                        ...
                        if not default_analytic_account:
                            raise Warning(
                                _("No analytic account for commission found!")
                            )
                                ...
                                commission = commission_obj.search(
                                    [
                                        (
                                            "invoice_line_id",
                                            "=",
                                            commission_line["invoice_line_id"],
                                        ),
                                        (
                                            "partner_id",
                                            "=",
                                            commission_line["partner_id"],
                                        ),
                                        (
                                            "product_id",
                                            "=",
                                            commission_line["product_id"],
                                        ),
                                        (
                                            "val_based",
                                            "=",
                                            commission_line.get("val_based", False),
                                        ),
                                    ]
                                )
                        ...
                        if sign > 0:
                            cr.execute(
                                "SELECT o.user_id, o.id FROM sale_order_line_invoice_rel rel  "
                        ...
                        # invoice based (refund)
                            ...
                            # order based
                            if analytic_account:
                                order = order_obj.with_context(
                                    active_test=False
                                ).search(
                                    [("project_id", "=", analytic_account.id)], limit=2
                                )
                                if len(order) == 1:
                                    if create_commission(
                                        order.user_id,
                            ...
                            # fallback invoice based
                            if not user_id and create_commission(
                                invoice.user_id,
                                commission_date=invoice.date_invoice,
                                ref=invoice.origin,
                            ):
                        ...
                        if analytic_account:
                            ...
                            if (
                                salesman_user
                                and not line.sale_order_line_ids
                                and (not user_id or user_id != salesman_user.id)
                            ):
