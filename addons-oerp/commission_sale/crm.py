...
class bonus(osv.osv):
    ...
    _columns = {
        ...
        "line_ids": fields.one2many(
            "commission_sale.bonus_line", "bonus_id", "Bonus Lines"
        ),
    }
...
class bonus_line(osv.osv):
    ...
    _columns = {
        ...
        "bonus_id": fields.many2one(
            "commission_sale.bonus",
            "Sales Bonus",
            required=True,
            select=True,
            ondelete="cascade",
        ),
        "volume_of_sales": fields.float(
            "Volume of Sales",
            digits_compute=dp.get_precision("Sale Price"),
            required=True,
        ),
        "bonus": fields.float(
            "Bonus %", help="Bonus = Volume of Sales * Bonus %", required=True
        ),
    }
...
class crm_case_section(osv.osv):
    ...
    _columns = {
        ...
        "commission_rule_ids": fields.one2many(
            "commission_sale.rule", "section_id", "Commission Rules"
        ),
        "sales_bonus_id": fields.many2one("commission_sale.bonus", "Sales Bonus"),
        "property_commission_product": fields.property(
            type="many2one", relation="product.product", string="Invoice Product"
        ),
        "property_analytic_journal": fields.property(
            type="many2one",
