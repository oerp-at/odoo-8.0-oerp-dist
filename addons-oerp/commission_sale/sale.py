...
class sale_order(osv.osv):
    ...
    _columns = {
        "commission_avg": fields.function(
            _commission_fields,
            type="float",
            string="Commission Average %",
            multi="_commission_fields",
            readonly=True,
        ),
        "commission_amount": fields.function(
            _commission_fields,
            type="float",
            string="Commission",
            multi="_commission_fields",
...
class sale_order_line(osv.osv):
    ...
    def _commission_fields(self, cr, uid, ids, field_names, arg, context=None):
        ...
        cr.execute(
            "SELECT l.order_id FROM sale_order_line l WHERE l.id IN %s GROUP BY 1",
            (tuple(ids),),
        )
        ...
        for order in order_obj.browse(cr, uid, order_ids, context=context):
            for commission_line in commission_task_obj._commission_sale(
                cr, uid, order, context=context
            ):
                commission_dict[commission_line["order_line_id"]].append(
                    commission_line
                )
    ...
    _columns = {
        ...
        "commission_custom": fields.float(
            "Custom Commission %",
            readonly=True,
        ...
        "commission_base": fields.function(
            _commission_fields,
            type="float",
