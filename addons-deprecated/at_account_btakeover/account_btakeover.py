...
class account_btakeover(osv.osv):
        ...
        for btakeover in self.browse(cr, uid, ids):
            for line in btakeover.line_ids:
                account_ids = account_obj.search(
                    cr,
                    uid,
                    [("code", "=", line.code), ("company_id", "=", user.company_id.id)],
                )
                account_id = account_ids and account_ids[0] or None
                if account_id:
                    balance = account_obj._compute_account(
                        cr, uid, [account_id], date_till=btakeover.date, context=context
                    )[account_id]
                    ...
                    raise osv.except_osv(
                        _("Error"), _("There is no account for the entered code!")
                    )
                name = str(
                ...
                btakeover_line_obj.write(
                    ...
                    uid,
                    line.id,
                    {"state": "assumed", "move_id": move_id, "balance_is": balance_is},
                    context,
                )
    ...
    _columns = {
        ...
        "state": fields.selection(
            [("draft", "Draft"), ("assumed", "Assumed")], "State", readonly=True
        ),
...
class account_btakeover_line(osv.osv):
    def copy_data(self, cr, uid, id, default=None, context=None):
        ...
        default.update(
            {"state": "draft", "move_id": None, "balance_is": 0.0,}
        )
        ...
        res = super(account_btakeover_line, self).copy_data(
        ...
        for line in self.browse(cr, uid, ids):
            ...
            if account_id:
                balance = account_obj._compute_account(
                    ...
                    uid,
                    [account_id],
                    date_till=btakeover_obj.browse(cr, uid, line.btakeover_id).date,
                    context=context,
                )[account_id]
    ...
    def onchange_balance_nominal(
        self, cr, uid, ids, balance_nominal, btakeover_id, code
    ):
    ...
    _columns = {
        ...
        "balance_nominal": fields.float(
            "Balance nominal", digits_compute=dp.get_precision("Account")
        ),
        "balance_is": fields.function(
            _balance_is,
            type="float",
            string="Balance is",
