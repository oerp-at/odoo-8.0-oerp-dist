...
class res_partner(osv.Model):
    ...
    _columns = {
        "education_ids": fields.one2many(
            "eagency.client.education", "partner_id", "Education"
        ),
        "skill_ids": fields.many2many(
            "eagency.skill",
            "eagency_partner_skill_rel",
            "partner_id",
            "skill_id",
            "Skills",
        ),
        ...
        "lang_skill_ids": fields.one2many(
            "eagency.lang.skill", "partner_id", "Language Skills"
        ),
        "lang_other": fields.char("Other language skills"),
        "preferred_area_ids": fields.many2many(
            "eagency.area",
            "eagency_partner_area_rel",
            "partner_id",
            "area_id",
            "Areas",
            required=True,
...
class eagency_client_education(osv.Model):
    ...
    _columns = {
        "partner_id": fields.many2one(
            "res.partner", "Client", invisible=True, required=True, ondelete="cascade"
        ),
        "completed": fields.integer("Graduation date", required=True),
        "country_id": fields.many2one(
            "res.country", "Graduation country", required=True
        ),
        "education_id": fields.many2one(
            "eagency.education", "Education", required=True
        ),
    }
