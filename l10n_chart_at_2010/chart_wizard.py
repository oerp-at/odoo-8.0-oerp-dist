...
class wizard_multi_charts_accounts(osv.osv_memory):
    def _create_bank_journals_from_o2m(
        self, cr, uid, obj_wizard, company_id, acc_template_ref, context=None
    ):
        """
        ...
        for line in journal_data:
            ...
            account_ids = obj_acc.search(
                cr,
                uid,
                [("code", "=", line["acc_name"]), ("company_id", "=", company_id)],
            )
            if not account_ids:
                raise osv.except_osv(
                    _("Configuration Error!"),
                    _("Account %s does not exist") % (line["acc_name"],),
                )
            ...
            vals_journal = self._prepare_bank_journal(
                cr, uid, line, current_num, account.id, company_id, context=context
            )
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if "bank_accounts_id" in fields:
            res.update(
                {
                    "bank_accounts_id": [
                        {"acc_name": "3135", "account_type": "bank"},
