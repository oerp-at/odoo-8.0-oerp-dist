...
class XmlWriter:
    ...
    def writeTaxRecord(self, fpos, src_tax, dest_tax):
        ...
        if dest_tax:
            self.write(
                '<field name="tax_dest_id" ref="%s"/>' % (self.getTaxID(dest_tax),)
            )
        self.exit()
        self.write("</record>")
        self.write("")
    ...
    def writeAccountRecord(self, fpos, src_account, dest_account):
        self.write(
            '<record id="'
            + self.getID(fpos + "_" + src_account)
            + '" model="account.fiscal.position.account.template">'
        )
        self.enter()
        self.write('<field name="position_id" ref="%s"/>' % (self.getID(fpos),))
        self.write(
            '<field name="account_src_id" ref="%s"/>'
            % (self.getAccountID(src_account),)
        )
        if dest_account:
            self.write(
                '<field name="account_dest_id" ref="%s"/>'
                % (self.getAccountID(dest_account),)
            )
        self.exit()
        self.write("</record>")
        self.write("")
