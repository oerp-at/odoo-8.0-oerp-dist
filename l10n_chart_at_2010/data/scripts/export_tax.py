...
class XmlWriter:
    ...
    def getID(self, inName):
        return "tax_%s" % (
            re.sub("[^A-Za-z0-9]", "", inName.lower().replace("steuer", "")),
        )
    ...
    def writeRecord(
        ...
        self.write(
            '<record id="' + self.getID(name) + '" model="account.tax.template">'
        )
        self.enter()
        ...
        if invoice_account:
            self.write(
                '<field name="account_collected_id" ref="%s"/>'
                % (self.getAccountID(invoice_account),)
            )
        if creditnote_account:
            self.write(
                '<field name="account_paid_id" ref="%s"/>'
                % (self.getAccountID(creditnote_account),)
            )
        if tax_base:
            self.write(
                '<field name="base_code_id" ref="%s"/>' % (self.getTaxCodeID(tax_base),)
            )
            self.write('<field name="base_sign">%s</field>' % (tax_base_sign,))
        if tax:
            self.write(
                '<field name="tax_code_id" ref="%s"/>' % (self.getTaxCodeID(tax),)
            )
            self.write('<field name="tax_sign">%s</field>' % (tax_sign,))
        if tax_credit_base:
            self.write(
                '<field name="ref_base_code_id" ref="%s"/>'
                % (self.getTaxCodeID(tax_credit_base),)
            )
            self.write(
                '<field name="ref_base_sign">%s</field>' % (tax_credit_base_sign,)
            )
        if tax_credit:
            self.write(
                '<field name="ref_tax_code_id" ref="%s"/>'
                % (self.getTaxCodeID(tax_credit),)
            )
