...
class hr_timesheet_sheet_sheet_day(osv.osv):
    def _total_saldo(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for daily_sheet in self.browse(cr, uid, ids, context):
            res[daily_sheet.id] = (daily_sheet.total_attendance or 0.0) - (
                daily_sheet.total_target or 0.0
            )
        return res
...
class hr_timesheet_sheet(osv.osv):
    def _build_sheet(
        ...
        for sheet in sheets:
            timesheet_data = self.get_timesheet_data(
                cr, sheet.user_id.id, sheet.id, context=context
            )
            ...
            while sheet_dt_cur <= sheet_dt_to and sheet_dt_cur <= dt_to:
                ...
                if timesheet_day:
                    ...
                    if sheet_dt_cur >= dt_from:
                        # get vars
                        day_total_timesheet = (
                            timesheet_day.get("total_timesheet_day") or 0.0
                        )
                        day_target = timesheet_day.get("total_target") or 0.0
                        day_attendance = (
                            timesheet_day.get("total_attendance_day") or 0.0
                        )
                        ...
                        if attendance_lines:
                            for attendance_line in attendance_lines:
                                ...
                                time_only = (
                                    timestamp_to and timestamp_to.split(" ")[1] or None
                                )
                        ...
                        # get work
                        work = timesheet_line_obj.browse(
                            cr, uid, timesheet_day.get("lines", []), context=context
                        )

                        # process leaves
                        leaves = timesheet_day.get("leaves", [])
                        leave_names = []
                        for leave in leave_obj.browse(
                            cr, uid, [l[0] for l in leaves], context=context
                        ):
    ...
    def get_timesheet_data(self, cr, uid, id, context=None):
        days = super(hr_timesheet_sheet, self).get_timesheet_data(
        ...
        for key, value in days.items():
            ...
            if contract:
                ...
                value["total_target"] = (
                    working_hour_obj.interval_hours_without_leaves(
                        cr,
                        uid,
                        contract["working_hours"].id,
                        dt,
                ...
                value["total_saldo"] = (
                    value.get("total_attendance_day") or 0.0
                ) - value["total_target"]
        return days
    ...
    def _current_saldo(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for sheet in self.browse(cr, uid, ids, context):
            if sheet.state == "new" or sheet.state == "draft":
                saldo = sheet.saldo_correction + (
                    sheet.total_attendance - sheet.total_target
                )
                ...
                while prev_ids:
                    ...
                    for prev_sheet_entry in prev_sheet_entries:
                        ...
                        if prev_sheet_entry["state"] in ("new", "draft"):
                            prev_sheet_saldo_correction = (
                                prev_sheet_entry["saldo_correction"] or 0.0
                            )
                            prev_sheet_total_attendance = (
                                prev_sheet_entry["total_attendance"] or 0.0
                            )
                            prev_sheet_total_target = (
                                prev_sheet_entry["total_target"] or 0.0
                            )
                            saldo += prev_sheet_saldo_correction + (
                                prev_sheet_total_attendance - prev_sheet_total_target
                            )
                        else:
    ...
    def _last_saldo(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for sheet in self.browse(cr, uid, ids, context=context):
            ...
            if prev_id:
                res[sheet.id] = (
                    self.read(
                        cr, SUPERUSER_ID, prev_id, ["current_saldo"], context=context
                    )["current_saldo"]
                    or 0.0
                )
        return res
    ...
    def _total_holiday(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for sheet in self.browse(cr, uid, ids, context):
            res[sheet.id] = {}
            status_ids = holidays_status_obj.search(
                cr, uid, [("categ_id.leave_type", "=", "holiday")]
            )
            days = holidays_status_obj.get_days_sum(
                cr, uid, status_ids, sheet.employee_id.id, context=context
            )
    ...
    def _total_sick_leaves(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for sheet in self.browse(cr, uid, ids, context):
            res[sheet.id] = {}
            status_ids = holidays_status_obj.search(
                cr, uid, [("categ_id.leave_type", "=", "sickness")]
            )
            days = holidays_status_obj.get_days_sum(
                cr, uid, status_ids, sheet.employee_id.id, context=context
            )
    ...
    def update_saldo(self, cr, uid, sid, context=None):
        ...
        for sheet in self.browse(cr, SUPERUSER_ID, sheet_ids, context=context):
            if sheet.saldo != sheet.current_saldo:
                self.write(
                    cr, uid, sheet.id, {"saldo": sheet.current_saldo}, context=context
                )
    ...
    _columns = {
        ...
        "saldo_correction": fields.float(
            "Correction", readonly=True, states={"draft": [("readonly", False)]}
        ),
        ...
        "total_current_target": fields.function(
            _total_current_target, string="Current Target"
        ),
        "max_leaves": fields.function(
            _total_holiday, string="Total Holidays", multi="total_holiday"
        ),
        "leaves_taken": fields.function(
            _total_holiday, string="Holiday Taken", multi="total_holiday"
        ),
        "remaining_leaves": fields.function(
            _total_holiday,
            string="Remaining Holidays",
            multi="total_holiday",
            help="Current remaining holidays, is calculated over all",
        ),
