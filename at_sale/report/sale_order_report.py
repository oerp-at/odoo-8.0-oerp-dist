...
class Parser(extreport.basic_parser):
    ...
    def prepare(self, sale_order):
        ...
        ids = obj_order_line.search(
            self.cr, self.uid, [("order_id", "=", sale_order.id)]
        )
        for sid in range(0, len(ids)):
            order = obj_order_line.browse(
                self.cr, self.uid, ids[sid], self.localcontext
            )
        ...
        for line in order_lines:
            ...
            res["product_uom"] = (
                line.product_uos and line.product_uos.name or line.product_uom.name
            )
