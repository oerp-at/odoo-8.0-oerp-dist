...
class crm_lead(models.Model):
    ...
    def _move_attachments(self, obj):
        ...
        if ids:
            ...
            attachments.write({"res_id": obj.id, "res_model": obj._model._name})

    def _move_messages(self, obj):
        for lead in self:
            for message in lead.message_ids:
                message.write({"model": obj._model._name, "res_id": obj.id})
    ...
    def _create_order(self, values=None):
        ...
        if shop_id:
            helper.onChangeValuesEnv(
                order_obj, values, order_obj.onchange_shop_id(shop_id, "draft", None)
            )
        ...
        if partner_id:
            helper.onChangeValuesEnv(
                ...
                order_obj.onchange_partner_id(
                    partner_id, shop_id=values.get("shop_id")
                ),
            )
