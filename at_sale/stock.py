...
class stock_picking(osv.osv):
    ...
    def _relids_picking(self, cr, uid, ids, context=None):
        if not ids:
            return []
        cr.execute(
            "SELECT m.picking_id FROM stock_move m WHERE m.id IN %s GROUP BY 1",
            (tuple(ids),),
        )
        res = [r[0] for r in cr.fetchall()]
        return res
    ...
    def _sale_id(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for oid in ids:
            res[oid] = {"sale_id": None, "shop_id": None}

        cr.execute(
            "SELECT p.id, s.id, s.shop_id FROM stock_picking p "
            " INNER JOIN sale_order s ON s.procurement_group_id = p.group_id "
            " WHERE p.id IN %s "
            " GROUP BY 1,2,3 "
            " ORDER BY p.id, s.date_order DESC, s.id DESC ",
            (tuple(ids),),
        )
