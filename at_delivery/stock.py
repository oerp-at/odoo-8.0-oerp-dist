...
class stock_picking(osv.Model):
    ...
    _columns = {
        "carrier_label_name": fields.char(
            "Carrier Label Name", copy=False, readonly=True
        ),
        ...
        "carrier_api": fields.related(
            "carrier_id", "api", type="char", string="Carrier API", readonly=True
        ),
        ...
        "carrier_order_id": fields.many2one(
            "delivery.order", "Delivery Order", ondelete="set null"
        ),
        "carrier_picking_id": fields.many2one(
            "stock.picking", "Added to Delivery", copy=False, readonly=True
        ),
    }
    ...
    def action_carrier_test(self, cr, uid, ids, context=None):
        for picking in self.browse(cr, uid, ids, context=None):
            if picking.carrier_api and not picking.carrier_status:
                self.action_carrier_label(
                    cr, uid, [picking.id], test=True, context=context
                )
                carrier_error = self.read(
                    cr, uid, picking.id, ["carrier_error"], context=context
                )["carrier_error"]
                if carrier_error:
    ...
    def action_done_from_ui(self, cr, uid, picking_id, context=None):
        next_picking_id = super(stock_picking, self).action_done_from_ui(
        ...
        if picking.carrier_api:
            ...
            if not picking.carrier_picking_id:
                if (
                    not picking.carrier_status
                    or picking.carrier_state == "created"
                    or not picking.carrier_label
                ):
