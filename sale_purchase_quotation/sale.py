...
class sale_order_line(osv.Model):
    ...
    def _unlink_purchase_line(self, cr, uid, unused_purchase_line_ids, context=None):
        ...
        if unused_purchase_line_ids:
            ...
            for purchase_line in purchase_line_obj.browse(
                cr, uid, unused_purchase_line_ids, context=context
            ):
                ...
                if len(purchase_order.order_line) == 1:
                    ...
                    if purchase_order.state == "draft" and not purchase_line.quot_sent:
                        purchase_order_obj.unlink(
                            cr, uid, [purchase_order.id], context=context
                        )
                    # cancel order
                    elif purchase_order.state in ("draft", "sent", "bid"):
                        purchase_order_obj.action_cancel(
                            cr, uid, [purchase_order.id], context=context
                        )
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            for supplier_info in supplier_infos:
                ...
                date_planned = util.dateToStr(
                    datetime.datetime.today() + datetime.timedelta(line.delay or 0.0)
                )
                ...
                if purchase_line_id:
                    ...
                    if purchase_order:
                        purchase_order_vals[
                            "picking_type_id"
                        ] = purchase_order.picking_type_id.id

                else:
                    purchase_order_vals[
                        "picking_type_id"
                    ] = purchase_order_obj._get_picking_in(cr, uid, context=context)
                ...
                #  update or create
                if purchase_line_id:
                    purchase_order_vals["order_line"] = [
                        (1, purchase_line_id, purchase_line_vals)
                    ]
                    ...
                    purchase_order_obj.create(
                        cr, uid, purchase_order_vals, context=context
                    )
            ...
            # update line
            self.write(
                cr, uid, [line.id], {"quotation_active": True, "quotation_id": False}
            )
    ...
    def _product_id_change(
        self, cr, uid, res, flag, product_id, partner_id, lang, context=None
    ):
        res = super(sale_order_line, self)._product_id_change(
            cr, uid, res, flag, product_id, partner_id, lang, context
        )
    ...
    def send_mail_supplier(self, cr, uid, ids, context=None):
        ...
        if not purchase_line_ids:
            raise osv.except_osv(
                _("Warning!"), _("E-mail was already sent to all supplier!")
            )

        # send mails
        return purchase_line_obj._send_supplier_mail(
    ...
    def _quotation_id(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context):
            ...
            if supplier:
                res[line.id] = purchase_line_obj.search_id(
                    cr,
                    uid,
                    [("sale_line_id", "=", line.id), ("partner_id", "=", supplier.id)],
                )
        return res
    ...
    def _product_margin(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if quotation and line.quotation_active:
                res[line.id] = (
                    line.price_subtotal
                    - quotation.price_subtotal
                    + self._product_margin_extra(cr, uid, line, context)
                )
            else:
                cur = line.order_id.pricelist_id.currency_id
                qty = (
                    line.product_uos and line.product_uos_qty
                ) or line.product_uom_qty
                tmp_margin = (
                    line.price_subtotal
                    - (line.purchase_price * qty)
                    + self._product_margin_extra(cr, uid, line, context)
                )
    ...
    def _relids_purchase_order_line(self, cr, uid, ids, context=None):
        cr.execute(
            "SELECT l.id FROM purchase_order_line pl "
    ...
    _columns = {
        "quotation_price": fields.function(
            _quotation_price,
            type="float",
            string="Cost Price",
            copy=False,
        ...
        "quotation_all": fields.function(
            _quotation_all, type="boolean", string="All Quotation Sent to Suppliers"
        ),
