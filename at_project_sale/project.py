...
class project(osv.osv):
    ...
    def name_search(
        ...
        res = super(project, self).name_search(
...
class task(osv.osv):
    ...
    def onchange_project(self, cr, uid, id, project_id, context=None):
        res = super(task, self).onchange_project(
            cr, uid, id, project_id, context=context
        )
        if project_id:
    ...
    _columns = {
        ...
        "inv_product_id": fields.many2one(
            "product.product",
            "Invoice Product",
            help="The product which will be used for timesheet based invoices",
        ),
    }
