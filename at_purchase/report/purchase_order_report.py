...
class Parser(report_sxw.rml_parse):
    ...
    def _get_delivery_info(self, purchase):
        if purchase.state in ("confirmed", "approved"):
            ...
            return [{"delivery_notes": ":".join(delivery_notes)}]
    ...
    def _get_line_tax(self, line_obj):
        ...
        res = [
            tax.name
            for tax in self.pooler.get_pool(self.cr.dbname)
            .get("account.tax")
            .browse(self.cr, self.uid, tax_ids)
        ]
        return ",\n ".join(res)

    def _get_tax(self, order_obj):
        self.cr.execute(
            "SELECT DISTINCT tax_id FROM purchase_order_taxe, purchase_order_line, purchase_order \
            WHERE (purchase_order_line.order_id=purchase_order.id) AND (purchase_order.id=%s)",
            (order_obj.id),
        )
        res = self.cr.fetchall() or None
        if not res:
        ...
        for tax in tax_obj.browse(self.cr, self.uid, tax_ids):
            self.cr.execute(
                "SELECT DISTINCT order_line_id FROM purchase_order_line, purchase_order_taxe \
                WHERE (purchase_order_taxe.tax_id=%s) AND (purchase_order_line.order_id=%s)",
                (tax.id, order_obj.id),
            )
            ...
            if lines:
                ...
                base = 0
                for line in (
                    self.pooler.get_pool(self.cr.dbname)
                    .get("purchase.order.line")
                    .browse(self.cr, self.uid, line_ids)
                ):
                    base += line.price_subtotal
                res.append(
                    {"code": tax.name, "base": base, "amount": base * tax.amount}
                )
        return res
